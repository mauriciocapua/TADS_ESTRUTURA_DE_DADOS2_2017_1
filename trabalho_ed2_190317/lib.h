...
lista* converter_lista(lista *p_lista, char *str, int i)
{
//    int i
//
...
//    }
    ...
    {
        insere_letra_fim(p_lista,retorna_str(str[i]));
        converter_lista(p_lista, str, i+1);
    }
    else
    {
        return p_lista;
...
//    int i;
    ...
    char* con(int qtd, int i,char *str, letra *tmp)
    {
        if(i<qtd)
        {
            str[i] = tmp->caracter;
...
lista *strconcat(lista *p_lista1, lista *p_lista2)
{
    ...
    lista *ins(int qtd, int i,lista *lista3, letra *tmp)
    {
        if(i<qtd)
        {
            insere_letra_fim(lista3,retorna_str(tmp->proximo->caracter));
            tmp = tmp->proximo;
            ins(qtd, i+1, lista3, tmp);
        }
        else{
            return lista3;
        }
...
//        int i;
        lista3 = ins(p_lista1->qtd, 0, lista3, tmp);
